...
delt = 0.1
e = KalmanFilter (dim_x=3, dim_z=1)
e.x = np.array([100., 10., 1.])
e.H = np.array([[1., 0., 0.]])
e.R = 5
...
def kinematic_Predict(delt):
    e.F = np.array([[1., delt, 0.5*(delt**2)],
                    [0., 1., delt],
                    [0., 0., 1.]])
    e.P *= 1000.
    e.Q = Q_discrete_white_noise(dim=3, dt=delt, var=0.13)
    ...
    return [e.predict(), f.predict(), g.predict()]
